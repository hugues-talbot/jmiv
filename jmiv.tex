...
The objectives of this section is to show both the potential of minimizing a purely digital Elastica energy but also to underline the difficulties of using it as is in a global combinatorial optimization framework.
...
\center
	\subfloat[\label{}]{%
	\includegraphics[scale=0.2]{images/local_search/definitions/regions2.eps}
	}\hspace{40pt}%
	\subfloat[\label{}]{%
...
It is easy to check that any two glued curves $g_1,g_2$ can be combined as long as its $C_2$ segments do not intersect. We denote $\mathcal{G}_{n,k}(C_1,C_2)$  the set of all $k$-combined $n$-glued curves. \jaco{(Is it useful if afterwards you never combine glued curves, ie $k=1$ after ?)} Finally, we define
...
The parameters $n,k$ are used to parameterize the neighborhood set. Let $S_O = \partial ( S \setminus R_1^-(S) ) $ and $S_I = \partial ( S \cup R_1^+(S) )$, the neighborhood set to shape $S$ is defined as

\begin{align*}
	\mathcal{N}(S,N,k) = \bigcup_{1 \leq n \leq N} int \big( \; \mathcal{G}_{n,k}(\partial S, S_I) \;\big) \cup int \big( \; \mathcal{G}_{n,k}(\partial S, S_O) \; \big),
\end{align*}

where $int(C)$ is the interior of the shape bounded by oriented curve $C$. Algorithm~\ref{alg:local-search} describes the local combinatorial process and Figure~\ref{fig:local-comb-square-results} presents the digital curve evolution when executing this algorithm for two different shapes with $k=1$ and $N=20$.
...
\begin{algorithm}
 ...
 \Input{A digital set $S$; the number of junctions $k$ to use; the maximum length of glued curves $N$; the maximum number of iterations \MIt; and a stop condition \Tol}
 \BlankLine
...
\begin{align}
	\hat{\kappa}^2(y) &= c1\Big( c2 - \sum_{x_i \in B_r(y) \cap S}{x_i} \Big)^2, 
...
In figure \ref{fig:m1-square-flow} we see some results for algorithm \ref{alg:evolution-model} with $m=1$. We observe a global movement towards roundness, but with several artifacts. We minimize the effects of a jaggy boundary by including a length penalization, weighed by coefficient $\beta$. Nonetheless, a higher estimation ball radius creates unstable shapes. In fact, the estimator is very sensitive in regions of low squared curvature, and its precisely in those regions which spurious pixels are included.


\begin{figure}[!ht]
\center
\begin{minipage}[b]{0.33\textwidth}
	\subfloat[$r=3, \beta=0$\label{}]{%
	\includegraphics[scale=0.2]{images/flow/length-radius-effect/triangle-r3-nolength/summary_flow.eps}
	}%
\end{minipage}%
\begin{minipage}[b]{0.33\textwidth}
	\subfloat[$r=3, \beta=0.15$\label{}]{%
	\includegraphics[scale=0.2]{images/flow/length-radius-effect/triangle-r3-length0.15/summary_flow.eps}
	}%
\end{minipage}%
\begin{minipage}[b]{0.33\textwidth}
	\subfloat[$r=5, \beta=0.15$\label{}]{%
...
In our experiments, the best results are obtained by executing algorithm \ref{alg:evolution-model} with $m=r$, where $r$ is the estimation ball radius. In plot \ref{fig:mx-elastica-plots} we observe how the elastica evolves along the iterations. If the radius is too large, the elastica increases starting from a critical point $p$. That's what happens for the triangle, in which the flow converges to a single point.
...
\center
\subfloat{%
		\includegraphics[scale=0.75]{images/flow/faster-high-curvature/0000.png}}%%1
		\vspace{15pt}
		\subfloat{%
		\includegraphics[scale=0.75]{images/flow/faster-high-curvature/0005.png}}%
\end{minipage}%
\begin{minipage}[b]{0.5\textwidth}
\center
\subfloat{%
...
\caption{High curvature regions evolves faster than lower ones (a). The flow can handle holes. }
\end{figure}
...
We present an application of the digital flow described in the previous section for supervised image segmentation. We include a data fidelity and length penalization terms in the energy. At iteration $k$, we minimize the following energy

\begin{align}			
	\min_{Y_k} \sum_{y \in Y_k}{\left( \alpha \cdot s(y) + \gamma \cdot g(y) \right)} + \beta \cdot \sum_{y \in R_r(S_k)}{\hat{\kappa}_{r}^2(y)}.
...
\end{align}
	...
	\begin{align*}
		s(y)=\sum_{y_i \in \mathcal{N}_4(y)}{ t(y_i) }, \quad \text{where } t(y_i) = \left\{\begin{array}{ll}
		(y-y_i)^2, & \text{if } y_i \in O_{r,i}\\
		(y-0), & \text{if } y_i \in F_{r,i}\\
		(y-1), & \text{otherwise }
		\end{array}\right.
	\end{align*}
	...
	Notice that data and length terms are defined in such a way in order to comply with algorithm \ref{alg:evolution-model}, in which we take the complement of the optimal solution. 
	
\begin{figure}
\begin{tabular}{ccccc}
& & $(\beta=0.5,\gamma=1)$ & $(\beta=1,\gamma=1)$ &\\
Seeds & \includegraphics[scale=0.25]{images/segmentation/coala/seeds.png} & 
		\includegraphics[scale=0.25]{images/segmentation/coala/r3_lg0_sq05_dt1_it20.png} & 
		\includegraphics[scale=0.25]{images/segmentation/coala/r3_lg0_sq1_dt1_it50.png}	&
		$r=3$\\
Only data & \includegraphics[scale=0.25]{images/segmentation/coala/r3_lg0_sq0_dt1_it20.png} & 
		\includegraphics[scale=0.25]{images/segmentation/coala/r7_lg0_sq05_dt1_it20.png} &
		\includegraphics[scale=0.25]{images/segmentation/coala/r7_lg0_sq1_dt1_it20.png} &
		$r=7$\\
\end{tabular}	
\caption{Influence of parameters in the segmentation.}
...
\begin{figure}
	\begin{minipage}[b]{0.32\textwidth}
		...
		\includegraphics[scale=0.2]{images/segmentation/tiger1/corrected-seg.png}		
	...
	\caption{Examples of segmentations.}
\end{figure}
...
\begin{figure}
	\center
	\subfloat{
		\includegraphics[scale=0.2]{images/segmentation/penguin/1/gc-seg.png}
	}%
	\subfloat{
		\includegraphics[scale=0.2]{images/segmentation/penguin/1/corrected-seg.png}			
	}%
	\subfloat{
		\includegraphics[scale=0.2]{images/segmentation/penguin/2/corrected-seg.png}			
	}%	
\end{figure}

Average of 3s per iteration. Average of 20 iterations. We optimize over a single band (inner pixel boundary).
