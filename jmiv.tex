...
\begin{align*}
	...
	&= \left\{ x \in \Omega \; | \; 0 < d_S(x) \leq m \right\} \; \cup \;  \left\{ x \in \Omega \; | \; 	0 < d_{\overline{S}}(x) \leq m \right\}\\
\end{align*}
...
The parameters $n,k$ are used to parameterize the neighborhood set. Let $S_O = \partial ( R_1^-(S) ) $ and $S_I = \partial ( S \cup R_1^+(S) )$, the neighborhood set to shape $S$ is defined as
