...
The digital version of elastica energy, using multigrid convergent estimators is defined as
\begin{align}
...
Given a digital shape $S\in\Omega$, its distance transformation $d_S$ and natural numbers $m > 0$, we define its $m$-ring set as
...
We define $O_{r}^{(i)}(y) = O^{(i)} \cap B_r(y)$ and an analogous definition holds for $F_{r}^{(i)}(y)$. Expanding \eqref{eq:curvature-estimator-pixels}, we get 
	...
	where $g(S,x)$ denotes a similarity term as distance to initial shape $S$ or data fidelity term and $s(x)$ denotes a length penalization term. Each choice of $m$ generates a different flow, which is generaly described in the DCE algorithm \ref{alg:evolution-model}.
...
Length and similarity terms should be properly defined in order to comply with the complement step of the DCE algorithm. The length penalization is defined as
