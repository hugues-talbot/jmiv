...
\section{Local combinatorial optimization \todo{(To be written, code is done)}}
    The goal of this section is to exemplify the use of digital estimator in optimization scheme in a simple
...
In this section we present a process that, given a digital shape $S_0$, generates a sequence $(S_i)$ of shapes with non-increasing elastica energy. The idea is to define a neighborhood $\mathcal{N}_i$ of $S_i$ and choose the element of $\mathcal{N}_i$ with lowest energy.  The process is suited for the integral invariant but also other curvature estimators, for example, MDCA \cite{roussillon11}.

Let $S$ a $2$-dimensional digital shape. We adopt the cellular-grid model to represent $S$, i.e., pixels and its lower dimensional counterparts, linels and pointels, are part of $S$. In particular, we denote by $\partial S$ the topological boundary of $S$, i.e., the connected sequence of linels such that for each linel we have one of its incident pixels in $S$ and the other not in $S$.


Let $d_{S}:\Omega \rightarrow \mathcal{R}$ be the euclidean distance transformation with respect to shape $S$. The value $d_S(x)$ gives the euclidean distance between $x$ and the closest background pixel in $\Omega$. 
...
\begin{align*}
\mathcal{U}(S) = \{ D \; | D \subset R_1(S) \cup S \; and \; \text{D is connected} \}
\end{align*}
...
An oriented closed curve $C$ is a sequence of linels, but it can also be seen as a sequence of pointels. In particular, the first pointel is also the last one. We write $p < q$ to denote that a pointel $p$ comes first than $q$ in some curve.
...
\begin{align*}
	\mathcal{G}_n(C_1,C_2) = \{ g=(a,p,q,b) \; | \; a,b \in C_1, \, p,q \in C_2, \, a<b, \, q-p=n \},
\end{align*}


\end{definition}

It is easy to check that any two glued curves $g_1,g_2$ can be combined as long as exists an orientation such that $a_2 > b_1$ and $b_2 < a_1$. We denote $\mathcal{G}_{n,k}(C_1,C_2)$  the set of all $k$-combined $n$-glued curves. Finally, we define
...
The parameters $n,k$ are used to parameterize the neighborhood set.

\begin{align*}
	\mathcal{N}(S,N,k) = \bigcup_{1 \leq n \leq N} \mathcal{G}_{n,k}(\partial S, S_I) \cup \mathcal{G}_{n,k}(\partial S, S_O)
\end{align*}

 Algorithm \ref{alg:local-search} describes the local combinatorial process and figures \ref{fig:local-comb-square-results} presents the results of executing the algoritmh for different shapes with $k=1$ and $n<20$.
...
\begin{algorithm}[H]
 ...
 \Input{A digital set $S$; the number of jonctions $k$ to use; the maximum length of glued curves $n$; the maximum number of iterations \MIt; and a stop condition \Tol}
 \BlankLine
...
The running time of algorithm \ref{alg:local-search} is summarized in table (). Although its use in practical applications is limited, we demonstrate that digital estimators are effective in its measurements and the evolved flows behave as expected. We observe that is a complete digital approach, and we do not suffer from discretization and rounding problems, a common issue in continuous models. In the next section, we explore a more efficient approach.
...
In our experiments we observe that we recover better results by using $m=r$, where $r$ is the estimation ball radius. In plots () we observe that the elastica energy value decreases along the iterations.
...
\center
		...
		\subfloat[$r=3,h=1.0$]{%
		\subfloat{%
		\includegraphics[scale=0.2]{images/flow/grid-radius-effect/triangle/r3-h1.0/summary_flow.eps}
		}%
		\hspace{15pt}
		\subfloat{%
		\includegraphics[scale=0.15]{images/flow/grid-radius-effect/square/r3-h1.0/summary_flow.eps}}%
		\hspace{15pt}
		\subfloat{%
\includegraphics[scale=0.2]{images/flow/grid-radius-effect/flower/r3-h1.0/summary_flow.eps}}%
		}
		
		\setcounter{subfigure}{-2}
		\subfloat[$r=5,h=1.0$]{%
		\subfloat{%
		\includegraphics[scale=0.2]{images/flow/grid-radius-effect/triangle/r5-h1.0/summary_flow.eps}
		}%
		\hspace{15pt}
		\subfloat{%
		\includegraphics[scale=0.15]{images/flow/grid-radius-effect/square/r5-h1.0/summary_flow.eps}}%
		\hspace{15pt}
		\subfloat{%
\includegraphics[scale=0.2]{images/flow/grid-radius-effect/flower/r5-h1.0/summary_flow.eps}}%
		}					
